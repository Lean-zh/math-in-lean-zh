...
1. **偏序集** 由集合 :math:`P` 以及定义在 :math:`P` 上的二元关系 :math:`\le` 组成，该关系具有传递性和自反性。

2. **群** 由集合 :math:`G` 以及其上的一个结合二元运算、一个单位元 :math:`1` 和一个将 :math:`G` 中每个元素 :math:`g` 映射为其逆元 :math:`g^{-1}` 的函数 :math:`g \mapsto g^{-1}` 构成。若运算满足交换律，则称该群为 **阿贝尔群** 或 **交换群** 。

3. **格** 是一种具有交和并运算的部分有序集。

4. **环** 由一个（加法表示的）阿贝尔群 :math:`(R, +, 0, x \mapsto -x)` 以及一个结合的乘法运算 :math:`\cdot` 和一个单位元 :math:`1` 组成，并且乘法对加法满足分配律。如果乘法是可交换的，则称环为 **交换环** 。

5. 一个 **有序环** :math:`(R, +, 0, -, \cdot, 1, \le)` 由一个环以及其元素上的一个偏序关系组成，满足以下条件：
   对于 :math:`R` 中的任意 :math:`a`、:math:`b` 和 :math:`c`，若 :math:`a \le b`，则 :math:`a + c \le b + c`；
   对于 :math:`R` 中的任意 :math:`a` 和 :math:`b`，若 :math:`0 \le a` 且 :math:`0 \le b`，则 :math:`0 \le a b`。

6. **度量空间** 由一个集合 :math:`X` 和一个函数 :math:`d : X \times X \to \mathbb{R}` 组成，满足以下条件：
  - 对于集合 :math:`X` 中的任意   :math:`x` 和 :math:`y`，有 :math:`d(x, y) \ge 0` 。
  - 当且仅当 :math:`x = y` 时，:math:`d(x， y) = 0` 。
  - 对于集合 :math:`X` 中的任意 :math:`x` 和 :math:`y` ，有 :math:`d(x， y) = d(y， x)` 。
  - 对于集合 :math:`X` 中的任意 :math:`x` 、:math:`y` 和 :math:`z` ，有 :math:`d(x, z) \le d(x, y) + d(y， z)` 。

7. **拓扑空间** 由集合 :math:`X` 以及 :math:`X` 的子集所构成的集合 :math:`\mathcal T` 组成，这些子集被称为 :math:`X` 的 **开子集** ，且满足以下条件：
  - 空集和 :math:`X` 是开集。
  - 两个开集的交集是开集。
  - 任意多个开集的并集是开集。

在上述每个例子中，结构的元素都属于一个集合，即 **载体集** ，有时它可代表整个结构。
例如，当我们说“设 :math:`G` 是一个群”，然后说“设 :math:`g \in G`”，
这里 :math:`G` 既代表结构本身，也代表其载体。
并非每个代数结构都以这种方式与单个载体集相关联。
例如， **二部图** 涉及两个集合之间的关系，**伽罗瓦连接** 也是如此。
...
其次，证明助手需要支持结构上的通用符号表示。
在 Lean 中，符号 ``*`` 用于所有常见数系中的乘法，也用于泛指群和环中的乘法。
当我们使用像 ``f x * y`` 这样的表达式时，
Lean 必须利用关于 ``f`` 、 ``x`` 和 ``y`` 的类型信息来确定我们所指的乘法运算。

第三，它需要处理这样一个事实，即结构可以通过多种方式从其他结构继承定义、定理和符号。
有些结构通过添加更多公理来扩展其他结构。
交换环仍然是环，因此在环中有意义的任何定义在交换环中也有意义，
任何在环中成立的定理在交换环中也成立。有些结构通过添加更多数据来扩展其他结构。
例如，任何环的加法部分都是加法群。
环结构添加了乘法和单位元，以及管理它们并将其与加法部分相关联的公理。
有时我们可以用另一种结构来定义一种结构。任何度量空间都有一个与之相关的标准拓扑，即 **度量空间拓扑** ，并且任何线性序都可以关联多种拓扑。

最后，重要的是要记住，数学使我们能够像使用函数和运算来定义数字一样，使用函数和运算来定义结构。
群的乘积和幂次仍然是群。
对于每个 :math:`n`，模 :math:`n` 的整数构成一个环，
对于每个 :math:`k > 0`，该环中系数的 :math:`k \times k` 多项式矩阵再次构成一个环。
因此，我们能够像计算其元素一样轻松地计算结构。
这意味着代数结构在数学中有着双重身份，既是对象集合的容器，又是独立的对象。
证明助手必须适应这种双重角色。

在处理具有代数结构相关联的类型的元素时，证明助手需要识别该结构并找到相关的定义、定理和符号。
这一切听起来似乎工作量很大，确实如此
。但 Lean 使用一小部分基本机制来完成这些任务。本节的目标是解释这些机制并展示如何使用它们。

第一个要素几乎无需提及，因为它太过显而易见：
从形式上讲，代数结构是 :numref:`section_structures` 中所定义的那种结构。
代数结构是对满足某些公理假设的数据束的规范，
我们在 :numref:`section_structures` 中看到，这正是 ``structure`` 命令所设计要容纳的内容。
这简直是天作之合！
