...
/- TEXT:
引入被全称量词限定的变量时可以使用任何名字，并非一定要是 ``x``, ``y`` 和 ``ε`` 。
隐式变量也必须被引入，因为隐式变量的意思是，当我们 **使用** ``my_lemma`` 时可以不写它们，但证明时还得写，因为它组成了待证命题。
在 ``intro`` 策略后，如果我们在冒号 **之前** 列出所有变量与假设，
目标就是一开始的样子，正如我们在上一节中所做的那样。
下面将看到为什么有时候有必要在证明开始之后引入变量和假设。
...
要 **使用** 单调性假设，你可以将其应用于合适的论证与假设，然后将得到的表达式应用于目标；
或者你可以将其直接应用于目标，并让 lean 将剩余假设显示为新的子目标，帮助你逆向推导。
BOTH: -/
section
variable (f g : ℝ → ℝ)
