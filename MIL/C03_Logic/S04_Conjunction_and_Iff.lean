...
/- TEXT:
**使用** 合取命题需要拆开两部分的证明。
你可以用 ``rcases`` 策略做这个，也可以使用 ``rintro`` 或一个模式匹配函数 ``fun`` ，使用方式和存在量词的情况类似。
TEXT. -/
-- QUOTE:
...
在 Lean 中， ``A ↔ B`` **并不** 定义为 ``(A → B) ∧ (B → A)`` ，但其实如果这样定义也无妨，而且它的行为几乎与此相同。
你可以输入 ``h.mp`` 和 ``h.mpr`` ，或者 ``h.1`` 和 ``h.2`` ，用于表示 ``h : A ↔ B`` 的两个方向。
你也可以使用 ``cases`` 及类似策略。要证明一条当且仅当语句，你可以使用 ``constructor`` 或角括号，就像你要证明一个合取一样。
TEXT. -/
-- QUOTE:
...
/- TEXT:
关于合取和双向蕴含的进一步练习。
**偏序** 是一种具有传递性、反身性和反对称性的二元关系。
更弱的概念 **预序** 只是一个反身、传递关系。
对于任何预序 ``≤`` ，Lean 把相应的严格预序公理化定义为 ``a < b ↔ a ≤ b ∧ ¬ b ≤ a`` 。
证明如果 ``≤`` 是偏序，那么 ``a < b`` 等价于 ``a ≤ b ∧ a ≠ b`` ：
TEXT. -/
-- BOTH:
section
-- QUOTE:
