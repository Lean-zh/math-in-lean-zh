...
我们通过 ``rcases`` 策略来使用形如 ``A ∨ B`` 的假设。
与在合取或存在量词中使用 ``rcases`` 不同，这里的 ``rcases`` 策略产生 **两个** 目标。
它们都有相同的结论，但在第一种情况下，``A`` 被假定为真，而在第二种情况下，``B`` 被假定为真。
换句话说，顾名思义， ``rcases`` 策略给出一个分情况证明。
像往常一样，我们可以把假设中用到的名字报告给 Lean。下例中我们告诉 Lean 对每个分支都使用名字 ``h`` 。
TEXT. -/
-- QUOTE:
...
/- TEXT:
模式从合取情况下的 ``⟨h₀, h₁⟩`` 变成了析取情况下的 ``h₀ | h₁`` 。
可以认为，第一种模式匹配包含 ``h₀`` **和** ``h₁`` 的数据，
而第二种模式，也就是带竖线的那种，匹配包含 ``h₀`` **或** ``h₁`` 的数据。
在这种情况下，因为两个目标分离，我们对两种情况可以使用同样的名字 ``h`` 。

绝对值函数的定义使得我们可以立即证明 ``x ≥ 0`` 蕴含着 ``|x| = x`` (这就是定理 ``abs_of_nonneg``)
而 ``x < 0`` 则蕴含着 ``|x| = -x`` （这是 ``abs_of_neg`` ）。
表达式 ``le_or_gt 0 x`` 推出 ``0 ≤ x ∨ x < 0`` ，使我们可以将这两种情况分开。
...
使用 ``cases`` 的好处是你可以以任意顺序证明每种情况；Lean 使用标签找到相关的目标。
如果你不在乎这个，你可以使用 ``next`` 或者 ``match`` ，甚至是模式匹配版的 ``have`` 。
TEXT. -/
-- QUOTE:
example : x < |y| → x < y ∨ x < -y := by
  cases le_or_gt 0 y
...
在 Mathlib 中， 这个事实被命名为 ``eq_zero_or_eq_zero_of_mul_eq_zero`` ，它是展示生产析取的另一个好例子。
使用它证明下列命题（使用 ``ring`` 策略简化计算）：
TEXT. -/
-- QUOTE:
...
/- TEXT:
在任意环 :math:`R` 中，对于元素 :math:`x`,
若存在非零元素 :math:`y` 使得 :math:`x y = 0` ，
...
具有这一性质的交换环称为 **整环** 。你对上面两个定理的证明在任意整环中应同样成立：
TEXT. -/
-- BOTH:
section
-- QUOTE:
