...
2. **群** 由集合 :math:`G` 以及其上的一个结合二元运算、一个单位元 :math:`1` 和一个将 :math:`G` 中每个元素 :math:`g` 映射为其逆元 :math:`g^{-1}` 的函数 :math:`g \mapsto g^{-1}` 构成。若运算满足交换律，则称该群为阿贝尔群或交换群。
...
7. **拓扑空间** 由集合 :math:`X` 以及 :math:`X` 的子集所构成的集合 :math:`\mathcal T` 组成，这些子集被称为 :math:`X` 的开子集，且满足以下条件：
  - 空集和 :math:`X` 是开集。
  - 两个开集的交集是开集。
  - 任意多个开集的并集是开集。
