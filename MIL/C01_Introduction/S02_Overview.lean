...
ccc每个表达式都有一个 **类型（Type）**，你可以使用 `#check` 命令来打印它。
一些数学对象表达式的类型可能是像 `ℕ` 或 `ℕ → ℕ` 这样的。
TEXT. -/
...
- Kevin Buzzard 正在领导这件伟大的事，
  详见：`The Fermat's Last Theorem Project <https://leanprover-community.github.io/blog/posts/FLT-announcement/>`_

本书配套教程 `Theorem Proving in Lean <https://leanprover.github.io/theorem_proving_in_lean4/>`_，
中文版 `Lean 中的定理证明 <https://www.leanprover.cn/tp-lean-zh/>`_，
提供了对Lean的基础逻辑框架和核心语法更全面的介绍。
**Lean 中的定理证明** 适用于那些在使用新洗碗机之前更喜欢从头到尾阅读用户手册的人。
如果你是那种更喜欢 **先动手尝试** ，以后再弄清细节的人，那么从本书开始更合适，需要时随时可以回去参考 **Lean 中的定理证明** 。

**Lean 形式化数学** 与 **Lean 中的定理证明** 的另一个区别在于，本书更加强调 **证明策略(Tactics)** 的使用。
...
/- TEXT:
以下是同一定理的 **策略式** 证明，其中以 ``--`` 开头的行是注释，因此被 Lean 忽略：
TEXT. -/
-- QUOTE:
...
/- TEXT:
当你在 VS Code 中输入上述证明的每一行时，Lean 会在一个单独的窗口中显示 **证明状态**，告诉你已经建立了哪些事实，
以及要证明你的定理还需要完成什么任务。
你可以通过逐行逐步骤回顾证明，因为 Lean 将继续显示光标所在点的证明状态。
本例中，证明的第一行引入了 ``m`` 和 ``n`` （此时可以重命名它们，如果我们想的话），
并且将假设 ``Even n`` 分解为 ``k`` 和假设 ``n = 2 * k``.
第二行， ``use m * k``, 声明我们将通过证明 ``m * n = 2 * (m * k)`` 来证明 ``m * n`` 是偶数。
下一行使用了 ``rw`` 策略（rewrite的缩写）在目标中将 ``n`` 替换为 ``2 * k``，
得到的新目标 ``m * (2 * k) = 2 * (m * k)`` 最终被 ``ring`` 策略解决。

逐步构建证明并获得增量反馈的能力非常强大。因此，策略证明通常比编写证明项更容易也更快。两者之间没有明显的区别：
策略证明可以插入到证明项中，
就像我们在上面的例子中使用短语 ``by rw [hk, mul_left_comm]`` 一样。
我们还将看到，反之，将一个简短的证明项插入到策略证明中通常也很有用。
...
/- TEXT:
两本入门教程之间的另一个重大区别是， **Lean 中的定理证明** 仅依赖于 Lean 内核以及其内置的策略，
而 **Lean 形式化数学** 建立在 Lean 强大且不断增长的库 **Mathlib** 的基础上。
...
交互式定理证明可能会令人沮丧，学习曲线很陡峭。但是 Lean 社区非常欢迎新人，
而且任何时间都有人在 `Lean Zulip 聊天群 <https://leanprover.zulipchat.com/>`_ 上在线回答问题。
