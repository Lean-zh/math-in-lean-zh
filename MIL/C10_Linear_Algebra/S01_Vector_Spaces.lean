...
简而言之，数学上我们想表达的是：拥有一个域 :math:`K` 上的向量空间结构意味着拥有一个加法交换群结构。虽然可以告诉 Lean 这一点，但如果这样做，每当 Lean 需要在类型 :math:`V` 上寻找该群结构时，它就会尝试去寻找带有 **完全未指定** 域 :math:`K` 的向量空间结构，而这个 :math:`K` 无法从 :math:`V` 中推断出来。
...
-- QUOTE.
/- TEXT:
现在我们转向任意族向量空间的直和与直积。这里我们将展示如何定义一个向量空间族，并访问直和与直积的泛性质。请注意，直和符号限定在 `DirectSum` 命名空间中，且直和的泛性质要求索引类型具备可判定等价性（这在某种程度上是实现上的偶然情况）。
EXAMPLES: -/
...
-- 直和的泛性质将从各个直和因子映射出的映射组装成从直和映射出的映射
