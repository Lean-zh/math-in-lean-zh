...
* **最终发生的事情** ，包括对于足够大的自然数 ``n : ℕ`` 发生的事情，或者在某一点 ``x`` 足够近的地方发生的事情，
  或者对于足够接近的点对发生的事情，或者在测度论意义上几乎处处发生的事情。
  对偶地，滤子也可以表达 **经常发生的事情** 的概念：对于任意大的 ``n`` ，在给定点的任意邻域内存在某点发生，等等。
...
将类型 ``X`` 上的一个滤子视为 ``Set X`` 的广义元素，可能更有用。
例如， ``atTop``  是“极大数的集合”，而 ``𝓝 x₀`` 是“非常接近 ``x₀`` 的点的集合”。
这种观点的一种体现是，我们可以将任何 ``s : Set X`` 与所谓的 **主滤子（principal filter）** 相关联，
该主滤子由包含 ``s`` 的所有集合组成。
...
上述的 ``Tendsto₁`` 概念在定义上等同于在 Mathlib 中定义的 ``Tendsto`` 概念，但后者定义得更为抽象。
``Tendsto₁`` 的定义存在的问题是它暴露了量词和 ``G`` 的元素，并且掩盖了通过将滤子视为广义集合所获得的直观理解。
我们可以通过使用更多的代数和集合论工具来隐藏量词 ``∀ V`` ，并使这种直观理解更加突出。
第一个要素是与任何映射 ``f : X → Y`` 相关的 **前推** 操作 :math:`f_*`，在 Mathlib 中记为 ``Filter.map f`` 。
给定 ``X`` 上的滤子 ``F`` ， ``Filter.map f F : Filter Y`` 被定义为使得 ``V ∈ Filter.map f F ↔ f ⁻¹' V ∈ F`` 成立。
在这个示例文件中，我们已经打开了 ``Filter`` 命名空间，因此 ``Filter.map`` 可以写成 ``map`` 。
这意味着我们可以使用 ``Filter Y`` 上的序关系来重写 ``Tendsto`` 的定义，该序关系是成员集合的反向包含关系。
...
请注意，根据滤子定义中的第二个性质（如果 ``U`` 属于 ``F`` ，那么任何包含 ``U`` 的集合也属于 ``F`` ），
第一个性质（ ``X`` 的所有元素组成的集合属于 ``F`` ）等价于 ``F`` 不是空集合这一性质。
这不应与更微妙的问题相混淆，即空集是否为 ``F`` 的一个 **元素** 。
滤子的定义并不禁止 ``∅ ∈ F`` ，但如果空集在 ``F`` 中，那么每个集合都在 ``F`` 中，
也就是说， ``∀ U : Set X, U ∈ F`` 。
在这种情况下， ``F`` 是一个相当平凡的滤子，恰好是完备格 ``Filter X`` 的最小元素。
这与布尔巴基对滤子的定义形成对比，布尔巴基的定义不允许滤子包含空集。

由于我们在定义中包含了平凡滤子，所以在某些引理中有时需要明确假设非平凡性。
不过作为回报，该理论具有更优的整体性质。我们已经看到，包含平凡滤子为我们提供了一个最小元素。
它还允许我们定义 ``principal : Set X → Filter X`` ，将 ``∅`` 映射到 ``⊥`` ，而无需添加预条件来排除空集。
而且它还允许我们定义拉回操作时无需预条件。实际上，有可能 ``comap f F = ⊥`` 尽管 ``F ≠ ⊥`` 。
例如，给定 ``x₀ : ℝ`` 和 ``s : Set ℝ`` ， ``𝓝 x₀`` 在从与 ``s`` 对应的子类型强制转换下的拉回非平凡
当且仅当 ``x₀`` 属于 ``s`` 的闭包。

为了管理确实需要假设某些滤子非平凡的引理，
Mathlib 设有类型类 ``Filter.NeBot`` ，并且库中存在假设 ``（F : Filter X）[F.NeBot]`` 的引理。
实例数据库知道，例如， ``(atTop : Filter ℕ).NeBot`` ，并且知道将非平凡滤子向前推进会得到一个非平凡滤子。
因此，假设 ``[F.NeBot]`` 的引理将自动应用于任何序列 ``u`` 的 ``map u atTop`` 。

我们对滤子的代数性质及其与极限的关系的探讨基本上已经完成，但我们尚未证明我们所提出的重新捕捉通常极限概念的主张是合理的。
表面上看， ``Tendsto u atTop (𝓝 x₀)`` 似乎比在 :numref: ``sequences_and_convergence``  中定义的收敛概念更强，
因为我们要求 ``x₀`` 的 **每个** 邻域都有一个属于 ``atTop`` 的原像，
而通常的定义仅要求对于标准邻域 ``Ioo (x₀ - ε) (x₀ + ε)`` 满足这一条件。
关键在于，根据定义，每个邻域都包含这样的标准邻域。这一观察引出了 **滤子基（filter basis）** 的概念。

给定 ``F : Filter X`` ，如果对于每个集合 ``U`` ，我们有 ``U ∈ F`` 当且仅当它包含某个 ``s i`` ，
那么集合族 ``s : ι → Set X`` 就是 ``F`` 的基。
换句话说，严格说来， ``s`` 是基当且仅当它满足 ``∀ U : Set X, U ∈ F ↔ ∃ i, s i ⊆ U`` 。
考虑在索引类型中仅选择某些值 ``i`` 的 ``ι`` 上的谓词会更加灵活。
对于 ``𝓝 x₀`` ，我们希望 ``ι`` 为 ``ℝ`` ，用 ``ε`` 表示 ``i`` ，并且谓词应选择 ``ε`` 的正值。
因此，集合 ``Ioo (x₀ - ε) (x₀ + ε)`` 构成 ``ℝ`` 上邻域拓扑的基这一事实可表述如下：
EXAMPLES: -/
-- QUOTE:
