...
/- TEXT:
所以我们现在知道如何 **证明** 一个存在语句。但我们要如何 **使用** 它？如果我们知道存在一个具有特定性质的对象，我们就可以为任意一个对象命名并对其进行推理。例如， 回顾上一节的谓词 ``FnUb f a`` 和 ``FnLb f a`` ，它们分别是指 ``a`` 是 ``f`` 的一个上界或下界。我们可以使用存在量词说明 “ ``f`` 是有界的”，而无需指定它的界：
TEXT. -/
-- BOTH:
-- QUOTE:
...
像 ``⟨a, ubfa⟩`` 这样长得像是一个匿名构造子的记号，称为 **模式（pattern）**，它们描述了我们在解包主参数时期望找到的信息。
给出假设 ``ubf``, 即 ``f`` 存在上界，``rcases ubf with ⟨a, ubfa⟩`` 在上下文中添加一个新变量 ``a`` 作为上界，
并添加假设 ``ubfa`` ，即该变量有给定的性质。
这有点像 ``intro`` ，目标没有变化， **但引入了** 新对象和新假设来证明目标。
这是数学推理的一种常规方法：我们解包对象，其存在性被一些假设断言或蕴含，然后使用它论证其他一些东西的存在性。
...
/- TEXT:
这个证明并未给出太多直觉洞见，现在我们展示证明思路。
**高斯整数** 是形如 :math:`a + bi` 的数，其中 :math:`a` 和 :math:`b` 是整数，而 :math:`i = \sqrt{-1}` 。
根据定义，高斯整数 :math:`a + bi` 的 **范数** 是 :math:`a^2 + b^2` 。
所以高斯整数的范数是平方和，且任意平方和都可以这样表示。
上述定理反映了一个事实，即高斯整数的乘积的范数等于范数的乘积：
若 :math:`x` 是 :math:`a + bi` 的范数，且 :math:`y` 是 :math:`c + di` 的范数，
则 :math:`xy` 是 :math:`(a + bi) (c + di)` 的范数。
我们充满惊人注意力的证明说明了这样一个事实：最容易形式化的证明并不总是最透彻的。
在 :numref:`section_building_the_gaussian_integers` 中，我们将为你介绍定义高斯整数的方法，并利用它们提供另一种证明。

在存在量词中解包等式，然后用它来重写目标中的表达式的模式经常出现，以至于 ``rcases`` 策略提供了一个缩写：
如果使用关键字 ``rfl`` 代替新的标识符，``rcases`` 就会自动进行重写（这一技巧不适用于模式匹配的 lambda）。
TEXT. -/
-- QUOTE:
theorem sumOfSquares_mul' {x y : α} (sosx : SumOfSquares x) (sosy : SumOfSquares y) :
    SumOfSquares (x * y) := by
...
另一个重要的例子是，
若函数 :math:`f : \alpha \to \beta` 满足对值域 :math:`\beta` 中任意的 :math:`y`,
存在定义域 :math:`\alpha` 中的 :math:`x`, 使得 :math:`f(x) = y` ，那么我们称这个函数是 **满射** 。
注意到这个语句既包含全称量词，也包含存在量词，这解释了为什么接下来的例子同时使用了 ``intro`` 和 ``use`` 。
TEXT. -/
-- BOTH:
section
