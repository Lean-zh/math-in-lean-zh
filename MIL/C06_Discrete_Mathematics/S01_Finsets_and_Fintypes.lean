...
Dealing with finite sets and types in Mathlib can be confusing,
because the library offers multiple ways of handling them.
In this section we will discuss the most common ones.

We have already come across the type ``Finset`` in :numref:`section_induction_and_recursion`  and :numref:`section_infinitely_many_primes` .
As the name suggests, an element of type ``Finset α`` is a finite set of elements of type ``α`` .
 We will call these “finsets.” The ``Finset`` data type is designed to have a computational interpretation,
  and many basic operations on ``Finset α`` assume that ``α`` has decidable equality,
  which guarantees that there is an algorithm for testing whether ``a : α`` is an element of a finset ``s`` .
