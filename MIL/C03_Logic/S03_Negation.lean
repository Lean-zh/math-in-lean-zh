...
/- TEXT:
第一、二、四个可以使用你已经学到的方法直接证明。鼓励你尝试。
然而，第三个更为困难，因为它是从一个对象的不存在可以得出矛盾的这一事实中得出结论，认为该对象是存在的。
这是 **经典** 数学推理的一个实例。我们可以像下面这样使用反证法证明第三个结果。
TEXT. -/
-- QUOTE:
example (h : ¬∀ x, P x) : ∃ x, ¬P x := by
  by_contra h'
...
我们以 **爆炸原理** （ex falso）结束本节，它是说从自相矛盾可以得出任何命题。
在 Lean 中，它用 ``False.elim`` 表示，它对于任何命题 ``P`` 确认了 ``False → P`` 。
这似乎是一个奇怪的原理，但它经常出现。我们经常通过分情况来证明定理，有时我们可以证明其中一种情况是矛盾的。
在这种情况下，我们需要断言矛盾确证了目标，这样我们就可以继续下一个目标了。
