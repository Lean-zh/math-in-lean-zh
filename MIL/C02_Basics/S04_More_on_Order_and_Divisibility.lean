...
注意到我们使用 ``min a b`` 而不是 ``min (a, b)`` 来将 ``min`` 应用于一对参数 ``a`` 和 ``b`` 。
从形式上讲，``min`` 是有两个参数的函数，类型为 ``ℝ → ℝ → ℝ`` ，箭头是右结合的，也就是 ``ℝ → (ℝ → ℝ)``。
其实际效果是，如果 ``a`` 和 ``b`` 的类型是 ``ℝ`` ，那么 ``min a`` 的类型是 ``ℝ → ℝ``，
而 ``min a b`` 的类型是 ``ℝ``。以这种方式处理多个参数被称为 **柯里化（currying）**，
以逻辑学家 Haskell Curry 的名字命名。

在 Lean 中，运算的顺序也可能需要一些时间来适应。
函数应用比中缀运算的优先级更高，因此表达式 ``min a b + c`` 被解释为 ``(min a b) + c``。
你会习惯的。

使用定理 ``le_antisymm`` ，我们可以证明两个实数如果彼此小于或等于对方，则它们相等。
利用这一点和上述的事实，我们可以证明 ``min`` 是可交换的：
TEXT. -/
-- QUOTE:
...
有趣的是， ``min`` 在 ``max`` 上的分配律就像乘法对加法的分配律一样，反之亦然。
换句话说，在实数上，我们有等式 ``min a (max b c) ≤ max (min a b) (min a c)``，
以及将 ``max`` 和 ``min`` 交换后的对应版本。
但在下一节中，我们将看到这并 **不** 是从 ``≤`` 的传递性和自反性以及上面列举的 ``min`` 和 ``max`` 的特性推导出来的。
我们需要使用实数上的 ``≤`` 是 **全序** 的事实，也就是说，它满足 ``∀ x y，x ≤ y ∨ y ≤ x`` 。
这里的析取符号， ``∨`` ，代表“或”。在第一种情况下，我们有 ``min x y = x``，
而在第二种情况下，我们有 ``min x y = y``。我们将在 :numref:`disjunction` 中学习如何根据情况来推理，
但现在我们只会使用不需要拆分情况的例子。
